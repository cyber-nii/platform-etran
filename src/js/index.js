...
const data = {
    all: {
        stats: [
            { label: 'Total Incidents', value: '1,245', change: '+12.5%', isPositive: false },
            { label: 'Resolved', value: '1,100', change: '+8.2%', isPositive: true },
            { label: 'Pending', value: '145', change: '-2.4%', isPositive: true },
            { label: 'Resolution Rate', value: '94.2%', change: '+0.1%', isPositive: true },
        ],
    ...
    zenith: {
        stats: [
            { label: 'Total Incidents', value: '450', change: '+5.5%', isPositive: false },
            { label: 'Resolved', value: '400', change: '+12.1%', isPositive: true },
            { label: 'Pending', value: '50', change: '-1.0%', isPositive: true },
            { label: 'Resolution Rate', value: '88.9%', change: '-0.2%', isPositive: false },
        ],
    ...
    gtb: {
        stats: [
            { label: 'Total Incidents', value: '320', change: '-15.2%', isPositive: true },
            { label: 'Resolved', value: '315', change: '+4.5%', isPositive: true },
            { label: 'Pending', value: '5', change: '-3.2%', isPositive: true },
            { label: 'Resolution Rate', value: '98.4%', change: '+0.1%', isPositive: true },
        ],
    ...
    access: {
        stats: [
            { label: 'Total Incidents', value: '475', change: '+2.1%', isPositive: false },
            { label: 'Resolved', value: '380', change: '-1.5%', isPositive: false },
            { label: 'Pending', value: '95', change: '+4.1%', isPositive: false },
            { label: 'Resolution Rate', value: '80.0%', change: '-0.6%', isPositive: false },
        ],
...
const renderStats = (stats) => {
    statsContainer.innerHTML = stats.map(stat => `
        <div class="bg-slate-800/50 rounded-xl p-6 border border-slate-700/50 shadow-lg backdrop-blur-sm hover:bg-slate-800/80 transition-all duration-300 group">
            <dt class="truncate text-sm font-medium text-slate-400 group-hover:text-sky-400 transition-colors">${stat.label}</dt>
            <dd class="mt-2 flex items-baseline sm:pb-7">
                <p class="text-2xl font-semibold text-white">${stat.value}</p>
                <p class="ml-2 flex items-baseline text-sm font-semibold ${stat.isPositive ? 'text-green-400' : 'text-red-400'}">
                    ${stat.isPositive ? '↑' : '↓'}
                    <span class="sr-only"> ${stat.isPositive ? 'Increased' : 'Decreased'} by </span>
                    ${stat.change}
                </p>
            </dd>
        </div>
...
const initCharts = () => {
    ...
    if (trendsChartEl) {
        ...
        deviceChartInstance = new Chart(ctxTrends, { // Reuse variable or rename later, keeping simple for now
            type: 'bar',
            data: {
